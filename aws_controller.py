...
def setup_worker(ip):
    ...
    if stdout.channel.recv_exit_status():
        _, stdout, _ = client.exec_command(
            "cd ~ && git clone https://github.com/edwjchen/circ_benchmarks.git && cd ~/circ && git checkout mpc_aws && python3 driver.py -F aby c lp bench && python3 driver.py -c && cd ~ && ./circ/aws_benchmark/setup.sh")
        if stdout.channel.recv_exit_status():
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if cmd_type == "help":
            ...
            logs()
        elif cmd_type in ["quit", "q", "exit", "exit()"]:
            sys.exit(0)
        else:
